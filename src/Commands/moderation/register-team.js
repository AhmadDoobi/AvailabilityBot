...
const {
  reloadTeamsAndGamesCommands,
} = require("../../Handlers/reload-teams-games-commands");
const {
  checkIfTeamAlreadyExists,
  checkIfGuildHasTeamForGame,
} = require("../../Functions/db-checks");
...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    const teamExistsOnAnotherServer = await checkIfTeamAlreadyExists(
      gameName,
      teamName,
      guildId
    );
    if (teamExistsOnAnotherServer) {
      await interaction.editReply({
        content:
          "The team name you tried to set is already connected to another server. \nif you think this is a mistake please contact <a7a_.>",
        ephemeral: true,
      });
      return;
    }
    const guildHasTeamForGame = await checkIfGuildHasTeamForGame(
      guildId,
      gameName
    );
